...
def start_stop_chr(start,stop,chr):
    res = requests.post("http://localhost:9200/lab/annotations/_search?pretty=true&size=50", json=
    {
        "query": {
            "bool": {
                "must": [
...
def gwas(coordinate,chr):
    gwas = []
    for i in range(0,len(coordinate)):
        res = requests.post("https://sysbio02.informatics.iupui.edu/elasticsearch/gwas/_search?pretty=true&scroll=10m&size=1",verify=False, json={
        "query": {
            "term": {"CHR_ID": chr[i]}
        },
        "sort" : {
            "_script" : {
                ...
                "script" : {
                    "lang": "painless",
                    "inline": "def factor = "+ str(coordinate[i]) +  "; def cur = 0; cur = (factor - doc['CHR_POS'].value); if (cur < 0) { cur = cur * -1 } else { cur = cur}"},
                "order" : "asc"
            }
        }
        })
...
def exon_sgrna_peek(bs_start,bs_stop,bs_chr,annotations,binding_site):
    ...
    gwas_final = gwas(bs_start,bs_chr)
    sgrna_final = []
    exon_final = []
    peek_final = []
    ...
    return annotations, binding_site, sgrna_final, peek_final,exon_final, gwas_final
...
def rna_bp(rbp, efficiency,specificity):
    res = requests.post("http://localhost:9200/lab/binding_site/_search?pretty=true&size=100", json=
    {
        "query": {
            "match": {
                "GENE": rbp.lower()
...
def gene_sg_rna(coordinate,efficiency,specificity,chr):
    ...
    for i in range(len(coordinate)):
        a = int(coordinate[i])
        res = requests.post("http://localhost:9200/lab/_search?pretty=true&scroll=10m&size=1", json={
        "query": {
            "bool": {
                "must":
        			{"match": {"BS_CHR" : chr[i]} },
    ...
    if request.method == "POST":
        if request.form.get("check") == "ZERO":
            ...
            if "|" in coordinates:
                ...
                annotations, bs, sgrna, peek, exons, gwas_value = start_stop_chr(coordinates[0], coordinates[1], str(chrom).lower())
                return render_template("result_table.html", annotations=annotations, bs=bs, sgrna=sgrna, peek=peek, exons=exons, length=len(bs), chrom=chrom, gwas_value=gwas_value)
            ...
            return render_template("rbp_result_table.html",bs=bs, sgrna=sgrna, peek=peek, exons=exons, length=length, gwas_value=gwas_value)
